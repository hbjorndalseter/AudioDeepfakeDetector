...
class ASVspoofLADataset(Dataset):
    ...
    def _load_audio_fixed(self, path: str) -> np.ndarray:
        ...
        - crop (random for train, center for dev/eval) if too long
        - apply random gain augmentation in training mode
        """
        ...
        # Random gain augmentation in training mode
        if self.train:
            gain = np.random.uniform(0.8, 1.2)   # ±20% amplitude variation
            y = y * gain
...
def main():
    ...
    ckpt_path = "checkpoints/resnet_logmel_pos1_gain_best.pth"
